...
export default defineConfig(({ mode }) => {
    ...
    // Get the app URL from .env or use a default
    const appUrl = env.VITE_APP_URL || env.APP_URL || 'http://localhost';
    
    // Force HTTPS for production URLs
    const assetUrl = isProduction ? appUrl.replace('http://', 'https://') : appUrl;
    ...
    return {
        ...
        // Add base path for production to ensure correct asset URLs
        ...(isProduction && { base: assetUrl })
    };
});
